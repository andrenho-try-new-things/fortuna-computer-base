...
static void __attribute__((always_inline)) inline inline_draw_pixel(uint16_t x, uint16_t y, Color color)
{
    // Range checks (640x480 display)
    // if((x > vga::screen_width - 1) | (x < 0) | (y > screen_height - 1) | (y < 0) ) return;
...
void draw_sprite(Sprite const& sprite, uint16_t x, uint16_t y)
{
    // TODO - deal with alignment issue
    if ((x & 1) == 0) {
        for (int m = 0; m < sprite.h; ++m) {
            const int pixel = ((screen_width * (y + m)) + x) ;
            memcpy(&vga_data_array[pixel>>1], &sprite.data[m * (sprite.w >> 1)], sprite.w >> 1);
        }
    } else {
        for (int m = 0; m < sprite.h; ++m) {
            for (int n = 0; n < sprite.w; n += 2) {
                const uint32_t idx = ((m * sprite.w) + n) >> 1;
                inline_draw_pixel(x + n, y + m, (Color) (sprite.data[idx] & 0xf));
                inline_draw_pixel(x + n + 1, y + m, (Color) (sprite.data[idx] >> 4));
            }
        }
    }
}
